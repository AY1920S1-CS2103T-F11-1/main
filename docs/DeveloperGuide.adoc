...
The logic component of Alfred handles the various commands within Alfred and the parsing of the user input to convert them into such commands. The logic component's primary structure is unchanged from the AB3 codebase, though it has been heavily modified to accommodate the new commands implemented with regards to Alfred, as can be seen from the class diagram below which highlights the high level structure of the logic component.
