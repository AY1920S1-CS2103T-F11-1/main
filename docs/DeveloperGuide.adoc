...
The `leaderboard` command fetches a leaderboard with teams sorted in descending order of their points, and ties between different teams broken *solely* on comparison-based methods (such as which teams has more participants) rather than the "random" method which will be covered later.
