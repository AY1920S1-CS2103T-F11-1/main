...
[discrete]
==== TODO: How the architecture components interact with each other to be updated with the later milestones.
...
==== Note: Model Architecture image to be updated.
...
.  The system is  not intended to be used with more than 1000 entities because student run hackathons rarely exceed this size and should not seem sluggish if the number of entites is less than 1500.
.  Project is not intended for use on mobile and only should be used on desktop.
.  The application is better ran from the command line rather than from the graphical user interface.
...
[[logging]] Logging::
Logging is the use of logging tools to track the performance of the application while it is running.
