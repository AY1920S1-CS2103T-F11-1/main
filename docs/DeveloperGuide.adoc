...
that will undergo negated search.
...
to filter the relevant `EntityList` by. These predicates are stored as static variables on a central `Predicates`
file. `FindParticipantCommand` selects the predicates based on which fields are provided in the original
input string.

4. With this predicate, it then calls the `findParticipant` method on the model with the generated predicate.
