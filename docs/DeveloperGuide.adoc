...
[[Design-Storage]]
=== Storage component
The Storage component handles the complexities of actually storing to and reading from disc the different data types.The Storage component transforms the AB3 implementation in order to support the storage of the 3
main EntityLists (ParticipantList, MentorList and TeamList). The 3 EntityLists form the main data objects
in Alfred, and Storage's purpose is to transform each of the EntityLists into a format that is JSON-Serializable
and store the data for each EntityList in separate JSON files.
...
1. `UserPrefs`: User Preferences for the Alfred Application (such as the Window Size of the application)
...
file, then Alfred will re-initialise the data type with an empty data type, persisting this newly initialised data type object
to disc. This is to prevent tampering of the data - inadvertent or malicious - from affecting the integrity
of the data in Alfred.
